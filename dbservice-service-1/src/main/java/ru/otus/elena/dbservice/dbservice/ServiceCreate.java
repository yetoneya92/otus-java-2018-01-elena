            ...
            while (result.next()) {
                ...
                for (int i = 0; i < fields.length - 1; i++) {
                    if (fields[i].getGenericType() instanceof ParameterizedType) {
                        ...
                        ArrayList<R> list=(ArrayList<R>)serviceExecution.findElements(id, fields[i], (DataSet)object);
                        coll.addAll(list);
                        ...
                        for (int j = 0; j < fields.length - 1; j++) {
                            if (columnName.contains(fields[j].getType().getSimpleName().toLowerCase())) {
                                if (parametrize) {
                                    ...
                                    element = (R) service.loadById(_id, (Class<R>) fields[j].getType()).getObject();
                                    if (element == null) {
                                        return null;
                                    }
