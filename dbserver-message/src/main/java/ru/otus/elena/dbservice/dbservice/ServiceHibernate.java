    ...
    public <T extends DataSet> ArrayList<String> save(T object) {
        ...
        try {
            try (Session session = sessionFactory.openSession()) {
                ...
                dao.save(object);
                messages.add(object.toString()+ "has been saved");
                return messages;
            }
        } catch (Exception e) {
            System.out.println("hasn't been saved: "+e.getMessage());
            messages.add(object.toString()+" hasn't been saved: "+e.getMessage());
            return messages;
        }
    }
